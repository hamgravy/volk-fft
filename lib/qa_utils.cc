...
#include <volk_fft/volk_fft_cpu.h>
#include <volk_fft/volk_fft_common.h>
#include <volk_fft/volk_fft_malloc.h>
...
void load_random_data(void *data, volk_fft_type_t type, unsigned int n) {
...
static std::vector<std::string> get_arch_list(volk_fft_func_desc_t desc) {
    std::vector<std::string> archlist;

    for(size_t i = 0; i < desc.n_impls; i++) {
        //if(!(archs[i+1] & volk_fft_get_lvarch())) continue; //this arch isn't available on this pc
...
volk_fft_type_t volk_fft_type_from_string(std::string name) {
    volk_fft_type_t type;
    type.is_float = false;
...
static void get_signatures_from_name(std::vector<volk_fft_type_t> &inputsig,
                                   std::vector<volk_fft_type_t> &outputsig,
                                   std::string name) {
    ...
    BOOST_FOREACH(std::string token, toked) {
        try {
            type = volk_fft_type_from_string(token);
...
inline void run_cast_test1(volk_fft_fn_1arg func, std::vector<void *> &buffs, unsigned int vlen, unsigned int iter, std::string arch) {
    while(iter--) func(buffs[0], vlen, arch.c_str());
}

inline void run_cast_test2(volk_fft_fn_2arg func, std::vector<void *> &buffs, unsigned int vlen, unsigned int iter, std::string arch) {
    while(iter--) func(buffs[0], buffs[1], vlen, arch.c_str());
}

inline void run_cast_test3(volk_fft_fn_3arg func, std::vector<void *> &buffs, unsigned int vlen, unsigned int iter, std::string arch) {
    while(iter--) func(buffs[0], buffs[1], buffs[2], vlen, arch.c_str());
}

inline void run_cast_test4(volk_fft_fn_4arg func, std::vector<void *> &buffs, unsigned int vlen, unsigned int iter, std::string arch) {
    while(iter--) func(buffs[0], buffs[1], buffs[2], buffs[3], vlen, arch.c_str());
}

inline void run_cast_test1_s32f(volk_fft_fn_1arg_s32f func, std::vector<void *> &buffs, float scalar, unsigned int vlen, unsigned int iter, std::string arch) {
    while(iter--) func(buffs[0], scalar, vlen, arch.c_str());
}

inline void run_cast_test2_s32f(volk_fft_fn_2arg_s32f func, std::vector<void *> &buffs, float scalar, unsigned int vlen, unsigned int iter, std::string arch) {
    while(iter--) func(buffs[0], buffs[1], scalar, vlen, arch.c_str());
}

inline void run_cast_test3_s32f(volk_fft_fn_3arg_s32f func, std::vector<void *> &buffs, float scalar, unsigned int vlen, unsigned int iter, std::string arch) {
    while(iter--) func(buffs[0], buffs[1], buffs[2], scalar, vlen, arch.c_str());
}

inline void run_cast_test1_s32fc(volk_fft_fn_1arg_s32fc func, std::vector<void *> &buffs, lv_32fc_t scalar, unsigned int vlen, unsigned int iter, std::string arch) {
    while(iter--) func(buffs[0], scalar, vlen, arch.c_str());
}

inline void run_cast_test2_s32fc(volk_fft_fn_2arg_s32fc func, std::vector<void *> &buffs, lv_32fc_t scalar, unsigned int vlen, unsigned int iter, std::string arch) {
    while(iter--) func(buffs[0], buffs[1], scalar, vlen, arch.c_str());
}

inline void run_cast_test3_s32fc(volk_fft_fn_3arg_s32fc func, std::vector<void *> &buffs, lv_32fc_t scalar, unsigned int vlen, unsigned int iter, std::string arch) {
    while(iter--) func(buffs[0], buffs[1], buffs[2], scalar, vlen, arch.c_str());
}
...
class volk_fft_qa_aligned_mem_pool{
public:
    void *get_new(size_t size){
        size_t alignment = volk_fft_get_alignment();
        void* ptr = volk_fft_malloc(size, alignment);
        memset(ptr, 0x00, size);
        _mems.push_back(ptr);
        return ptr;
    }
    ~volk_fft_qa_aligned_mem_pool() {
        for(unsigned int ii = 0; ii < _mems.size(); ++ii) {
            volk_fft_free(_mems[ii]);
        }
    }
private: std::vector<void * > _mems;
};

bool run_volk_fft_tests(volk_fft_func_desc_t desc,
                    void (*manual_func)(),
                    std::string name,
                    volk_fft_test_params_t test_params,
                    std::vector<volk_fft_test_results_t> *results,
                    std::string puppet_master_name
)
{
    return run_volk_fft_tests(desc, manual_func, name, test_params.tol(), test_params.scalar(),
...
bool run_volk_fft_tests(volk_fft_func_desc_t desc,
    ...
    if(results) {
        results->push_back(volk_fft_test_results_t());
    ...
    volk_fft_qa_aligned_mem_pool mem_pool;

    //now we have to get a function signature by parsing the name
    std::vector<volk_fft_type_t> inputsig, outputsig;
    get_signatures_from_name(inputsig, outputsig, name);

    //pull the input scalars into their own vector
    std::vector<volk_fft_type_t> inputsc;
    ...
    BOOST_FOREACH(volk_fft_type_t sig, inputsig) {
    ...
    std::vector<volk_fft_type_t> both_sigs;
    ...
    for(size_t i = 0; i < arch_list.size(); i++) {
        ...
        switch(both_sigs.size()) {
            case 1:
                if(inputsc.size() == 0) {
                    run_cast_test1((volk_fft_fn_1arg)(manual_func), test_data[i], vlen, iter, arch_list[i]);
                } else if(inputsc.size() == 1 && inputsc[0].is_float) {
                    if(inputsc[0].is_complex) {
                        run_cast_test1_s32fc((volk_fft_fn_1arg_s32fc)(manual_func), test_data[i], scalar, vlen, iter, arch_list[i]);
                    } else {
                        run_cast_test1_s32f((volk_fft_fn_1arg_s32f)(manual_func), test_data[i], scalar.real(), vlen, iter, arch_list[i]);
                    }
                } else throw "unsupported 1 arg function >1 scalars";
                break;
            case 2:
                if(inputsc.size() == 0) {
                    run_cast_test2((volk_fft_fn_2arg)(manual_func), test_data[i], vlen, iter, arch_list[i]);
                } else if(inputsc.size() == 1 && inputsc[0].is_float) {
                    if(inputsc[0].is_complex) {
                        run_cast_test2_s32fc((volk_fft_fn_2arg_s32fc)(manual_func), test_data[i], scalar, vlen, iter, arch_list[i]);
                    } else {
                        run_cast_test2_s32f((volk_fft_fn_2arg_s32f)(manual_func), test_data[i], scalar.real(), vlen, iter, arch_list[i]);
                    }
                } else throw "unsupported 2 arg function >1 scalars";
                break;
            case 3:
                if(inputsc.size() == 0) {
                    run_cast_test3((volk_fft_fn_3arg)(manual_func), test_data[i], vlen, iter, arch_list[i]);
                } else if(inputsc.size() == 1 && inputsc[0].is_float) {
                    if(inputsc[0].is_complex) {
                        run_cast_test3_s32fc((volk_fft_fn_3arg_s32fc)(manual_func), test_data[i], scalar, vlen, iter, arch_list[i]);
                    } else {
                        run_cast_test3_s32f((volk_fft_fn_3arg_s32f)(manual_func), test_data[i], scalar.real(), vlen, iter, arch_list[i]);
                    }
                } else throw "unsupported 3 arg function >1 scalars";
                break;
            case 4:
                run_cast_test4((volk_fft_fn_4arg)(manual_func), test_data[i], vlen, iter, arch_list[i]);
                break;
            default:
    ...
    for(size_t i=0; i<arch_list.size(); i++) {
        ...
        if(i != generic_offset) {
            for(size_t j=0; j<both_sigs.size(); j++) {
                ...
                if(fail) {
                    volk_fft_test_time_t *result = &results->back().results[arch_list[i]];
                    result->pass = !fail;
                    fail_global = true;
