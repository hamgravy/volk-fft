...
import optparse
import volk_fft_arch_defs
import volk_fft_machine_defs

def do_arch_flags_list(compiler):
    output = list()
    for arch in volk_fft_arch_defs.archs:
...
def do_machines_list(arch_names):
    output = list()
    for machine in volk_fft_machine_defs.machines:
...
def do_machine_flags_list(compiler, machine_name):
    output = list()
    machine = volk_fft_machine_defs.machine_dict[machine_name]
